...
from GUI_PG.object_detection.core import keypoint_ops
from GUI_PG.object_detection.core import standard_fields as fields
from GUI_PG.object_detection.utils import shape_utils
from GUI_PG.compare_scores import compare_scores
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for i in range(boxes.shape[0]):
    ...
    if scores is None or scores[i] > min_score_thresh:
      ...
      s.append(scores[i])
      if instance_masks is not None:#NONE
        box_to_instance_masks_map[box] = instance_masks[i]
      if instance_boundaries is not None:#NONE
  ...
  base = box_to_color_map
  # print(base)
  a = list(box_to_color_map.keys())
  b = list(box_to_color_map.values())
  del_list = compare_scores(b,s,a)
  del_list = set(del_list)

  for var in del_list:
      # print("判定された数値", var)
      del box_to_color_map[var]
  ...
  return image, class_name, base, s, boxes_val
